...
static NSString *const actionAtItemEnd = @"actionAtItemEnd";
...
@property (nonatomic, strong) RCTVideoPlayerViewController *playerViewController;
@property (nonatomic, strong) NSURL *videoURL;
@property (nonatomic, strong) RCTEventDispatcher *eventDispatcher;
@property (nonatomic, assign) BOOL playbackRateObserverRegistered;
@property (nonatomic, assign) BOOL isExternalPlaybackActiveObserverRegistered;
@property (nonatomic, assign) BOOL videoLoadStarted;
...
-(void)addPlayerTimeObserver
{
	...
	__weak AbloVideo *weakSelf = self;
	self.timeObserver = [self.player addPeriodicTimeObserverForInterval:CMTimeMakeWithSeconds(progressUpdateIntervalMS, NSEC_PER_SEC)
														  queue:NULL
...
- (void)dealloc
{
	...
	[self.player removeObserver:self forKeyPath:externalPlaybackActive context: nil];
...
- (void)addPlayerItemObservers
{
	...
	[self.playerItem addObserver:self forKeyPath:playbackBufferEmptyKeyPath options:0 context:nil];
	[self.playerItem addObserver:self forKeyPath:playbackLikelyToKeepUpKeyPath options:0 context:nil];
...
- (void)removePlayerItemObservers
{
	if (self.playerItemObserversSet) {
		...
		[self.playerItem removeObserver:self forKeyPath:playbackBufferEmptyKeyPath];
		[self.playerItem removeObserver:self forKeyPath:playbackLikelyToKeepUpKeyPath];
...
- (void)setSrc:(NSDictionary *)source
{
	...
	dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t) 0), dispatch_get_main_queue(), ^{
		...
		[self playerItemForSource:source withCallback:^(AVPlayerItem *playerItem) {
			...
			[weakSelf.playerItem addObserver:self forKeyPath:actionAtItemEnd options:0 context:nil];
			[weakSelf addPlayerItemObservers];
			[weakSelf setFilter:_filterName];
			[weakSelf setMaxBitRate:_maxBitRate];
			
			[weakSelf.player pause];
			
			if (weakSelf.playbackRateObserverRegistered) {
				[weakSelf.player removeObserver:self forKeyPath:playbackRate context:nil];
				weakSelf.playbackRateObserverRegistered = NO;
			}
			if (weakSelf.isExternalPlaybackActiveObserverRegistered) {
				[weakSelf.player removeObserver:self forKeyPath:externalPlaybackActive context:nil];
				weakSelf.isExternalPlaybackActiveObserverRegistered = NO;
			}
			...
			[weakSelf attachListeners];
			if (@available(iOS 10.0, *)) {
				weakSelf.player.automaticallyWaitsToMinimizeStalling = NO;
			}
			
			[weakSelf.player addObserver:self forKeyPath:playbackRate options:0 context:nil];
			weakSelf.playbackRateObserverRegistered = YES;
			
			[weakSelf.player addObserver:self forKeyPath:externalPlaybackActive options:0 context:nil];
			weakSelf.isExternalPlaybackActiveObserverRegistered = YES;
			
			[weakSelf addPlayerTimeObserver];
			
			//Perform on next run loop, otherwise onVideoLoadStart is nil
			if (weakSelf.onVideoLoadStart) {
				id uri = [source objectForKey:@"uri"];
				id type = [source objectForKey:@"type"];
				weakSelf.onVideoLoadStart(@{
					@"src": @{
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
	...
	if([keyPath isEqualToString:actionAtItemEnd]) {
		NSLog(@"reached end...");
	}
...
- (void)setFullscreen:(BOOL) fullscreen {
	...
	else if ( !fullscreen && _fullscreenPlayerPresented )
	{
		...
		[_presentingViewController dismissViewControllerAnimated:true completion:^{
			[self videoPlayerViewControllerDidDismiss:weakSelf.playerViewController];
		}];
	}
}
...
- (void)usePlayerViewController
{
	...
	{
		if (!_playerViewController) {
			self.playerViewController = (RCTVideoPlayerViewController*)[self createPlayerViewController:_player withPlayerItem:_playerItem];
		}
...
- (void)setProgressUpdateInterval:(Float64)progressUpdateInterval
{
